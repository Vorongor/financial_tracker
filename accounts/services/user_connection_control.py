...
class UserInvitationService:
    ...
    @classmethod
    def approve_user_to_connect(cls, connection_id: int) -> None:
        connection = UserConnection.objects.get(pk=connection_id)
        connection.status = UserConnection.Status.ACCEPTED
        connection.save()

    @classmethod
    def reject_user_to_connect(cls, connection_id: int) -> None:
        UserConnection.objects.get(pk=connection_id).delete()

    @classmethod
    def block_user_to_connect(cls, connection_id: int) -> None:
        connection = UserConnection.objects.get(pk=connection_id)
        connection.status = UserConnection.Status.BLOCKED
        connection.save()

    @classmethod
    def un_block_user_connect(cls, connection_id: int) -> None:
        connection = UserConnection.objects.get(pk=connection_id)
        connection.status = UserConnection.Status.ACCEPTED
        connection.save()
